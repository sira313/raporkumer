...
export const GET: RequestHandler = async ({ url, locals }) => {
      ...
      if (Number.isInteger(id)) {
        ...
        if (muridIds.length && mapelIds.length) {
          const sumatif = await db.query.tableAsesmenSumatif.findMany({
            ...
            where: and(inArray(tableAsesmenSumatif.muridId, muridIds), inArray(tableAsesmenSumatif.mataPelajaranId, mapelIds))
          });
  ...
  const kepala = s?.kepalaSekolah ? { nama: s.kepalaSekolah.nama || '', nip: s.kepalaSekolah.nip || '' } : null;
