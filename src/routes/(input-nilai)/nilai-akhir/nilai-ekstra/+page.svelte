<script lang="ts">
    ...
    const daftarEkstrakurikuler = $derived(status === 'ready' ? (data.daftarEkstrakurikuler ?? []) : []);
                        ...
                        {#each daftarEkstrakurikuler as item (item.id)}
                            <tr>
                                ...
                                <td>
                                    ...
                                    <div class="text-xs text-base-content/60">{item.tujuanCount ?? 0} tujuan</div>
                                </td>
                                <td class={`font-semibold${item.sudahDinilai ? '' : ' text-base-content/60 italic'}`}>
                    ...
                    {:else}
                        <tr>
                            <td colspan="4" class="p-8 text-center">
                                <em class="opacity-60">Belum ada data ekstrakurikuler untuk kelas ini atau murid belum dinilai.</em>
                            </td>
                        </tr>
                    {/if}
                </tbody>
    ...
    {:else}
        <div class="alert alert-soft alert-warning mt-4 flex items-start gap-3">
            ...
            <span>
                ...
                <p class="text-base-content/70 text-sm">Gunakan tombol "Lihat" pada halaman Rekap Nilai Akhir untuk membuka rincian.</p>
            </span>
        </div>
    {/if}
</div>
