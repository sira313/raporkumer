<script lang="ts">
  ...
  rows.push(header);
      ...
      if (muridList.length) {
        ...
        for (const m of muridList) {
          ...
          for (const h of (headers.length ? headers : subjectCols.map((s, i) => ({ id: i, nama: s })))) {
      ...
      try {
        ...
        for (const r of mergePairs) {
          try {
            ...
            const chosen = (bottomVal !== null && bottomVal !== undefined && bottomVal !== '') ? bottomVal : topVal;
            ws.mergeCells(r);
  ...
  for (let c = 1; c <= lastCol; c++) {
    ...
    try {
      ...
      ws.getCell(cellAddr).alignment = { horizontal: 'centerContinuous', vertical: 'middle', wrapText: false } as any;
    } catch (e) {
  ...
  headerRow.font = { bold: true } as any;
      ...
      try {
        ...
        for (let c = subjectStartCol; c <= subjectEndCol; c++) {
          ...
          cell.alignment = { textRotation: 90, wrapText: true, vertical: 'bottom', horizontal: 'center' } as any;
  ...
  const studentStartRow = 6; // header is row 5 (we added Intrakurikuler at row 4), students start at row 6
        ...
        for (let i = 0; i < studentCount; i++) {
          ...
          try {
            row.getCell(capaianCol).alignment = { wrapText: true, vertical: 'middle', horizontal: 'center' } as any;
          } catch (e) {
        ...
        try {
          rataRow.getCell(capaianCol).alignment = { wrapText: true, vertical: 'middle', horizontal: 'center' } as any;
        } catch (e) {
          // ignore
        }
          ...
          try {
            ...
            for (const colIndex of specialCols) {
              // header alignment
              headerRow.getCell(colIndex).alignment = { vertical: 'middle', horizontal: 'left' } as any;
              // column default alignment if available
              if (ws.columns && ws.columns[colIndex - 1]) {
                ws.columns[colIndex - 1].alignment = { vertical: 'middle', horizontal: 'left' } as any;
              }
            }
            // specifically enable wrapText for the Capaian Kelas header/column
            try {
              headerRow.getCell(capaianCol).alignment = { vertical: 'middle', horizontal: 'left', wrapText: true } as any;
              if (ws.columns && ws.columns[capaianCol - 1]) {
                ws.columns[capaianCol - 1].alignment = Object.assign({}, ws.columns[capaianCol - 1].alignment || {}, { wrapText: true });
              }
            } catch (e) {
              // ignore
            }
...
<button class="btn btn-primary btn-soft shadow-none" on:click={downloadLeger} title="Download Leger (Excel)">Download Leger (.xlsx)</button>
