...
async function main() {
  ...
  try {
      ...
      try {
        ...
        if (cols.some((c) => String(c).toLowerCase() === 'username_normalized')) {
          try {
            ...
            console.info(`[fix-drizzle-indexes] Dropped index that referenced username_normalized: ${name}`);
          } catch (err) {
            console.warn(`[fix-drizzle-indexes] Failed to drop index ${name}:`, err && (err.message || err.toString()));
          }
        }
      } catch (_) {
    ...
    try {
      await client.execute({ sql: `CREATE UNIQUE INDEX IF NOT EXISTS "auth_user_username_normalized_unique" ON "auth_user" ("username_normalized")` });
      console.info('[fix-drizzle-indexes] Ensured canonical index auth_user_username_normalized_unique exists');
    } catch (err) {
