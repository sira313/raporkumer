...
- Endpoint masuk `POST /api/sinkronisasi/synchronizer` (`routes/api.php:34`) — dipakai aplikasi eksternal
  **Synchronizer v2** mendorong data *ke dalam* e-Rapor; diautentikasi middleware `auth.apikey`
...
*"Pengambilan Dapodik ditutup. Proses Pengambilan Dapodik hanya melalui aplikasi Synchronizer v.2"*.
...
3. `POST /sinkronisasi/matev-rapor` mengambil daftar *mata evaluasi* dari Dapodik:
...
| Aspek | Nilai |
|---|---|
| Header wajib | `Authorization: Bearer {token_dapodik}` (token statis dari menu *Web Service* aplikasi Dapodik desktop) |
...
|---|---|---|---|---|---|
| 1 | WebService Dapodik desktop (Alur A) | outbound GET & POST | **Bearer token** | `Authorization: Bearer {token_dapodik}` | Token WebService dari aplikasi Dapodik desktop sekolah; disimpan admin ke tabel `settings` (`key='token_dapodik'`, scoped per `sekolah_id`), dibaca ulang via `get_setting('token_dapodik', $sekolah_id)` |
...
2. **Token bersifat statis tanpa refresh flow.** Tidak ada endpoint login/renew terhadap Dapodik
   dalam repo ini; token dibuat/di-reset manual oleh operator di menu *Web Service* aplikasi
...
- Kredensial yang dibutuhkan hanya dua: `url_dapodik` (base URL WebService, biasanya
  ...
  (dari menu *Web Service* aplikasi Dapodik).
...
2. **Guru mengisi nilai**: lewat halaman *Nilai Akademik* — sumatif lingkup materi (per TP/KD),
...
3. **Kirim nilai ke Dapodik (satu-satunya jalur yang portable)**: konfigurasikan URL + Token
   ...
   *metadata mata evaluasi* (`postMatevRapor`, seluruh kolom `dapodik.matev_rapor`) dan *satu angka
   nilai akhir per siswa* (`postNilai`: `nilai_id`, `id_evaluasi`, `anggota_rombel_id`,
...
> Bagian ini menjawab: *"data apa saja yang harus disimpan sistem pengganti?"* Semua pemetaan
